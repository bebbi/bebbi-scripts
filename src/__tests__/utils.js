...
test('resolveBebbiScripts resolves to src/index.js when in the bebbi-scripts package', () => {
  mockPkg({package: {name: 'bebbi-scripts'}})
  expect(require('../utils').resolveBebbiScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.'),
  )
})

test('resolveBebbiScripts resolves to bebbi-scripts if not in the bebbi-scripts package', () => {
  mockPkg({package: {name: 'not-bebbi-scripts'}})
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolveBebbiScripts()).toBe('bebbi-scripts')
})
